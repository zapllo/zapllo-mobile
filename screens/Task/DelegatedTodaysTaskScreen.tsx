...
const DelegatedTodaysTaskScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const { todaysTasks } = route.params; // Safely access pendingTasks
  ...
  console.log('delayeeedddd', todaysTasks);
  ...
  return (
    <SafeAreaView className="h-full flex-1 bg-primary">
      ...
      <View className="flex h-20 w-full flex-row items-center justify-between p-5">
        ...
        <Text className="h-full pl-4 text-2xl font-semibold text-[#FFFFFF]">Today's Task</Text>
        <ProfileButton />
      </View>
