...
type Props = StackScreenProps<DelegatedTaskStackParamList, 'InTimeTask'>;
type InTimeTaskScreenRouteProp = RouteProp<DelegatedTaskStackParamList, 'InTimeTask'>;
...
const DelegatedInTimeTask: React.FC<Props> = ({ navigation }) => {
  const route = useRoute<InTimeTaskScreenRouteProp>();
  const { inTimeTasks } = route.params;
  ...
  const [filteredTasks, setFilteredTasks] = useState<any[]>(inTimeTasks);
  ...
  const [activeFilter, setActiveFilter] = useState('Category');
    ...
    const [customEndDate, setCustomEndDate] = useState<Date | null>(null);
      ...
      useEffect(() => {
              ...
              const dateRange = getDateRange(selectedTeamSize,inTimeTasks,customStartDate,customEndDate);
              ...
              const filteredByDate = filterTasksByDate(inTimeTasks, dateRange);
      ...
      const handleCustomDateApply = (startDate: Date, endDate: Date) => {
          ...
          const customFilteredTasks = filterTasksByDate(inTimeTasks, customDateRange);
  ...
  const applyFilter = () => {
    let tasksMatchingFilters = inTimeTasks;
  ...
  return (
    <SafeAreaView className="h-full flex-1 bg-primary">
      <View className="flex h-20 w-full flex-row items-center justify-between p-5">
        ...
        <Text className="h-full pl-4 text-2xl font-semibold text-[#FFFFFF]">In Time Tasks</Text>
        <ProfileButton />
      </View>
...
export default DelegatedInTimeTask;
