...
const DelegatedPendingTask: React.FC<Props> = ({ navigation }) => {
  ...
  const searchedTasks = useMemo(() => {
    return filteredTasks.filter((task: any) => {
      ...
      return (
        ...
        task.assignedUser?.firstName.toLowerCase().includes(searchLower) ||
        task.assignedUser?.lastName.toLowerCase().includes(searchLower) ||
  ...
  const getFilterBackgroundColor = (filter: string) => {
    return activeFilter === filter ? '#37384B' : '#0A0D28';
  };
...
export default DelegatedPendingTask;
